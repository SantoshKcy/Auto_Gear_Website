...
const Navbar = () => {
  ...
  const dropdownRef = useRef(); // New ref for the dropdown
  ...
  const ProfileDropdown = () => (
    <div ref={dropdownRef} className="absolute right-4 top-16 w-52 bg-[#1E1E1E] border border-gray-700 rounded-md shadow-lg z-[10000]" style={{ zIndex: 10000 }}>
      <NavLink
        to="/profile"
  ...
  return (
    <nav className="bg-[#101010]/90 border-b border-gray-700 text-white py-4 px-8 flex items-center justify-between top-0 w-full backdrop-blur-sm z-[2000]">
      {/* Logo */}
        ...
        {[
          ...
          <li key={item.to}>
            <NavLink
              ...
              className={({ isActive }) =>
                `${isActive
                  ? "text-white border-b-[3px] border-[#FF4500]"
                  : "text-[#E0E0E0] border-b-[3px] border-transparent hover:text-white hover:border-[#FF4500]"
      ...
      <div className="flex items-center space-x-2 text-[18px] relative" ref={profileRef}>
        {isAuthenticated ? (
          <>
            ...
            {showProfileMenu && createPortal(<ProfileDropdown />, document.body)}
          </>
        ) : (
          <>
            <NavLink
