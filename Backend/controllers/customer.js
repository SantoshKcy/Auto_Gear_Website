...
const { protect, authorize } = require("../middleware/auth"); // Import RBAC Middlewar
const fs = require("fs");
...
exports.register = asyncHandler(async (req, res, next) => {
    ...
    const customer = await Customer.create({
        fname, lname, phone, email: email.trim().toLowerCase(), password, image,
        role: role || "customer",
    });
    ...
    const confirmationUrl = `http://localhost:5173/confirm-email/${confirmationToken}`;
...
exports.confirmEmail = asyncHandler(async (req, res, next) => {
    ...
    // ✅ Only confirming, not logging in
...
exports.login = asyncHandler(async (req, res, next) => {
    ...
    if (!customer.isVerified) return res.status(403).json({ message: "Please verify your email before logging in" });
...
exports.forgotPassword = asyncHandler(async (req, res, next) => {
    ...
    const resetUrl = `http://localhost:5173/reset-password/${resetToken}`; // not port 3000
